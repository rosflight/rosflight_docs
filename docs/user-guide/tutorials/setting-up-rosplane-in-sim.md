...
!!! tip
    This document will focus more on getting ROSplane up and running, not an in-depth discussion of the architecture.
    For more in-depth description of the ROSplane autonomy stack, see the [ROSplane user-guide pages](../rosplane/index.md).
...
At its core, ROSplane enables users to fly waypoint missions that are specified by the user.

When you launch the ROSplane autonomy stack later in the tutorial, you will launch many different ROS2 nodes, each of which has its own responsibility.
Here's a list of the main nodes in the ROSplane autonomy stack and what each does:

- **Path Planner**: Interfaces with user to add or clear waypoints.
- **Path Manager**: Converts waypoints into flyable paths between waypoints using fillets or Dubins curves
- **Path Follower**: Follows straight lines or circles generated by the path manager
- **Controller**: Controls the aircraft. In other words, move motors to achieve some airspeed/altitude/course commands.
- **Estimator**: Estimates the state (i.e. position, velocity, attitude) based on sensor information


ROSplane implements a cascaded control structure.
All this means is that high-level waypoint commands flow from the path planner, to the path manager, to the path follower, and finally to low-level control loops.
See the [ROSplane user guide](../rosplane/index.md) for more information and some diagrams.

As we launch ROSplane and load waypoints to ROSplane's path planner, you will see all of these nodes in action.

## Launching `standalone_sim`

First, launch the `standalone_sim` from [the previous tutorial](./manually-flying-rosflight-sim.md).
The command is repeated here for convenience.
...
This command should do two things: open RViz (the standalone simulator), and VimFly (for [flying around in sim](./manually-flying-rosflight-sim.md)).
...
!!! warning "Configuring the firmware for flight"
    If you forgot to load parameters to the firmware, or if you didn't write the firmware parameters, make sure you [review the previous tutorial](./manually-flying-rosflight-sim.md).

    Without properly configuring the firmware, you won't get ROSplane to work!
...
You should see the following nodes in the output:
```bash
➜  ~ ros2 node list
/controller
/estimator
...
Let's now take a look at some of the topics specific to `rosplane`:
...
??? info "**Some Key Topic Descriptions**"
    Remember that a ROS2 `topic` is a particular "channel" with specific data types.
    Any ROS2 node can send information with that data type across that channel to any other ROS2 node.

    Here are some of the important topics in our ROS2 network: 
...
!!! tip "Exploring data"
    ROS2 provides some great tools for "introspection" when nodes are running.
    To see a live print out of all the data being sent over a particular channel on the ROS2 network, use
    ```bash
    ros2 topic echo <topic-name-goes-here>
    ```
    Go ahead and echo some topics to experiment with what data is being sent over the network.
    What data is being sent over each node, and why might nodes need to know this information?

    Those questions are best answered if you know the details of each node, so this is mostly just a thought experiment to get you thinking.
...
When we publish waypoints, we'll want to visualize them so we can see where the plane is flying.
...
We're now ready to start loading and flying a waypoint mission!
...
!!! tip
    You can easily define your own mission YAML file and put it anywhere you like.
    Just make sure to modify the above service call to have the correct path of your file.
...
!!! warning
    Make sure to **first** arm, **then** disable override.
    ROSflight will not allow you to arm the vehicle after disabling RC override.

    In other words, the RC safety pilot has to have control (i.e. RC override enabled) over the vehicle for ROSflight to arm.

After a few seconds, you should see the airplane take off from the ground, climb, and then turn to follow the waypoint mission.
By default, ROSplane will just repeatedly loop between the waypoints you have specified.
Make sure to [read more about each ROSplane node](../rosplane/index.md) and how to configure it.


### Monitor Flight Progress

In ROSplane, every module communicates with the other modules via ROS2 publishers and subscribers.
...
??? tip "How do I know what information is contained in a message definition?"
    ...
    ```bash
    ros2 interface show rosflight_msgs/srv/SetSimState
    ```
    and look through the output.
    This will tell you what information is included in a message definition.
...
- [ROSplane Architecture Documentation](./index.md): In-depth system design and implementation details
