...
Here's a list of the main nodes in the ROSplane autonomy stack and what it does:

- **Path Planner**: High-level mission planning and waypoint management
- **Path Manager**: Convert waypoints into flyable paths between waypoints using fillets or Dubins curves
- **Path Follower**: Follow straight lines or circles generated by the path manager
- **Controller**: Control the aircraft. In other words, move motors to achieve some airspeed/altitude/course commands.
- **Estimator**: Estimate the state (i.e. position, velocity, attitude) based on sensor information
...
!!! tip "Exploring data"
    ...
    ```
    Go ahead and echo some topics to experiment what data is being sent over the network.
...
!!! warning
    ...
    ROSflight will not allow you to arm the vehicle before disabling RC override.
